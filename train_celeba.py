...
def main():
    ...
    input_size = (h, w)
    dataset = CelebADataSet(args.data_dir, max_iters=None, crop_size=input_size, 
    ...
    while run:
        ...
        for idx in pbar:
            ...
            if global_iteration % args.save_pred_every == 0 or global_iteration >= args.num_steps:
                ...
                torch.save(seg_model.state_dict(),osp.join(args.snapshot_dir, 'CelebA_scenes_'+str(global_iteration)+'.pth'))

            if global_iteration == 1:
                torch.save(seg_model.state_dict(),osp.join(args.snapshot_dir, 'CelebA_scenes_init.pth'))
